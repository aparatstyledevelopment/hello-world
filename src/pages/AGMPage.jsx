...
export function AGMPage() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-black">AGM Intelligence</h1>
      ...
      <div className="rounded-lg bg-black p-6">
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <p className="text-[11px] font-medium uppercase tracking-[0.1em] text-gray-500">GOVERNANCE INVESTORS</p>
            <p className="font-mono text-xl font-bold text-white mt-1">{govInvestors.length}</p>
          </div>
          <div>
            <p className="text-[11px] font-medium uppercase tracking-[0.1em] text-gray-500">ENGAGEMENTS DONE</p>
            <p className="font-mono text-xl font-bold text-gray-400 mt-1">{engagementsCompleted}</p>
          </div>
          <div>
            <p className="text-[11px] font-medium uppercase tracking-[0.1em] text-gray-500">OPEN SIGNALS</p>
            <p className="font-mono text-xl font-bold text-gray-400 mt-1">{openGovSignals}</p>
          </div>
        </div>
      </div>

      {/* ── Process Roadmap (Vertical Timeline) ──────────── */}
      <Card variant="section" accentColor="gray" title="Process Roadmap" subtitle="AGM preparation phases and milestones">
              ...
              <div key={phase.id} className="relative flex gap-5 pb-8">
                ...
                {!isLast && (
                  <div
                    className={cn(
                      ...
                      isCompleted ? "bg-gray-400" : isCurrent ? "bg-gray-400" : "bg-gray-200"
                    )}
                  />
                )}
                ...
                <div className="relative z-10 flex-shrink-0">
                  {isCompleted ? (
                    ...
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gray-700 text-white ring-4 ring-gray-100 animate-pulse">
                      <Clock size={16} />
                    </div>
                  ) : (
                ...
                <div className={cn(
                  "flex-1 min-w-0",
                  isCurrent && "rounded-lg border-2 border-gray-200 bg-gray-100/50 p-4 -mt-1"
                )}>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className={cn(
                      "text-sm font-semibold",
                      isCompleted ? "text-gray-500" : isCurrent ? "text-black" : "text-gray-700"
                    )}>
                      {phase.label}
                    </h3>
                    <span className={cn(
                    ...
                    <span className="font-mono text-xs text-gray-400 ml-auto">{phase.date}</span>
                  </div>
                  <p className={cn(
                  ...
                  {phase.action && (
                    <button className="mt-3 inline-flex items-center gap-1.5 rounded-lg bg-black px-4 py-2 text-xs font-semibold uppercase tracking-wider text-white hover:bg-gray-800 transition-colors">
                      <Eye size={13} />
                      {phase.action}
                    </button>
                  )}
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[11px] font-medium uppercase tracking-[0.1em] text-gray-400">CLASS A (1 VOTE/SHARE)</span>
              <span className="font-mono text-sm font-bold text-black">{classAVotes}%</span>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[11px] font-medium uppercase tracking-[0.1em] text-gray-400">CLASS B (10 VOTES/SHARE)</span>
              <span className="font-mono text-sm font-bold text-black">{classBVotes}%</span>
            </div>
            <div className="h-4 w-full rounded-full bg-gray-100">
              <div
                className="h-4 rounded-full bg-gray-600 transition-all"
      ...
      <div>
        <h2 className="text-[11px] font-medium uppercase tracking-[0.1em] text-gray-400 mb-3">
                ...
                return (
                  <tr key={inv.id}>
                    <td className="px-4 py-2.5">
                      <Link
                        to={`/investors/${inv.id}`}
                        className="font-medium text-black hover:text-gray-600 hover:underline"
                      >
                        {inv.name}
                      </Link>
                    </td>
      ...
      <Card variant="section" accentColor="slate" title="Historical Voting Results" subtitle="AGM voting outcomes 2023-2025">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {historicalVoting.map((row) => (
                <tr key={row.year}>
                  <td className="px-4 py-2.5 font-semibold text-black font-mono">
                    {row.year}
                  </td>
                  <td className="px-4 py-2.5">
                    <span className="flex items-center gap-1.5">
                      <span className={cn("h-2 w-2 rounded-full", row.boardElection >= 90 ? "bg-black" : "bg-gray-400")} />
                      <span className="font-mono font-medium text-gray-700">{row.boardElection}%</span>
                    </span>
                  </td>
                  <td className="px-4 py-2.5">
                    <span className="flex items-center gap-1.5">
                      <span className={cn("h-2 w-2 rounded-full", row.compensation >= 75 ? "bg-black" : row.compensation >= 60 ? "bg-gray-400" : "bg-red-500")} />
                      <span className="font-mono font-medium text-gray-700">{row.compensation}%</span>
                    </span>
                  </td>
                  <td className="px-4 py-2.5">
                    <span className="flex items-center gap-1.5">
                      <span className="h-2 w-2 rounded-full bg-black" />
                      <span className="font-mono font-medium text-gray-700">{row.auditorRatification}%</span>
                    </span>
                  </td>
                  <td className="px-4 py-2.5">
                    <span className="flex items-center gap-1.5">
                      <span className={cn("h-2 w-2 rounded-full", row.shareholderProposal >= 50 ? "bg-red-500" : "bg-gray-400")} />
                      <span className="font-mono font-medium text-gray-700">{row.shareholderProposal}%</span>
                    </span>
                  </td>
                </tr>
              ))}
        ...
        {/* Trend alert */}
        <div className="mt-4 rounded-lg bg-black text-white p-5">
          ...
          <p className="text-sm leading-relaxed">
            ...
            <span className="font-mono font-semibold text-gray-400">78.5%</span> to{" "}
