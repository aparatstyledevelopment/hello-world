...
const statusColors = {
  ...
  "In progress": "text-zinc-600",
  Planned: "text-zinc-600",
    ...
    <div className="p-4 md:p-6 space-y-8">
      ...
      <div className="rounded-2xl bg-zinc-900 p-6 shadow-sm">
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-zinc-500">GOVERNANCE INVESTORS</p>
            <p className="font-mono text-xl font-bold text-white mt-1">{govInvestors.length}</p>
          </div>
          <div>
            <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-zinc-500">ENGAGEMENTS DONE</p>
            <p className="font-mono text-xl font-bold text-zinc-400 mt-1">{engagementsCompleted}</p>
          </div>
          <div>
            <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-zinc-500">OPEN SIGNALS</p>
            <p className="font-mono text-xl font-bold text-zinc-400 mt-1">{openGovSignals}</p>
          </div>
        </div>
      </div>

      {/* ── Process Roadmap (Vertical Timeline) ──────────── */}
      <Card variant="section" accentColor="zinc" title="Process Roadmap" subtitle="AGM preparation phases and milestones">
              ...
              <div key={phase.id} className="relative flex gap-5 pb-8">
                ...
                {!isLast && (
                  <div
                    className={cn(
                      ...
                      isCompleted ? "bg-zinc-400" : isCurrent ? "bg-zinc-400" : "bg-zinc-200"
                    )}
                  />
                )}
                ...
                <div className="relative z-10 flex-shrink-0">
                  {isCompleted ? (
                    ...
                    <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-zinc-700 text-white ring-4 ring-zinc-100 animate-pulse">
                      <Clock size={16} />
                    </div>
                  ) : (
                    <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-zinc-200 text-zinc-400">
                ...
                <div className={cn(
                  "flex-1 min-w-0",
                  isCurrent && "rounded-2xl border-2 border-zinc-200/60 bg-zinc-100/50 p-4 -mt-1"
                )}>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className={cn(
                      "text-sm font-semibold",
                      isCompleted ? "text-zinc-500" : isCurrent ? "text-zinc-900" : "text-zinc-700"
                    )}>
                      {phase.label}
                    </h3>
                    <span className={cn(
                    ...
                    <span className="font-mono text-xs text-zinc-400 ml-auto">{phase.date}</span>
                  </div>
                  <p className={cn(
                  ...
                  {phase.action && (
                    <button className="mt-3 inline-flex items-center gap-1.5 rounded-xl bg-zinc-900 px-4 py-2 text-xs font-semibold uppercase tracking-wider text-white shadow-sm hover:bg-zinc-800 transition-colors">
                      <Eye size={13} />
                      {phase.action}
                    </button>
                  )}
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[11px] font-medium uppercase tracking-[0.08em] text-zinc-400">CLASS A (1 VOTE/SHARE)</span>
              <span className="font-mono text-sm font-bold text-zinc-900">{classAVotes}%</span>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[11px] font-medium uppercase tracking-[0.08em] text-zinc-400">CLASS B (10 VOTES/SHARE)</span>
              <span className="font-mono text-sm font-bold text-zinc-900">{classBVotes}%</span>
            </div>
            <div className="h-4 w-full rounded-full bg-zinc-100">
              <div
                className="h-4 rounded-full bg-zinc-500 transition-all"
      ...
      <div>
        <h2 className="text-[11px] font-medium uppercase tracking-[0.08em] text-zinc-400 mb-3">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {governanceIssues.map((issue) => (
            ...
            >
              ...
              <div className="border-t border-zinc-100 pt-3 mt-5">
            ...
            <thead>
              <tr className="border-b border-zinc-200 bg-zinc-50/50">
                {["INVESTOR", "HOLDING", "SENSITIVITY", "ISS ALIGNMENT", "BOARD VOTE", "COMP VOTE", "ENGAGEMENT"].map((col) => (
                  <th key={col} className="px-4 py-2.5 text-left text-[11px] font-medium uppercase tracking-[0.08em] text-zinc-400">
                    {col}
                  </th>
                ))}
              </tr>
      ...
      {/* ── Historical Voting ─────────────────────────────── */}
      <Card variant="section" accentColor="zinc" title="Historical Voting Results" subtitle="AGM voting outcomes 2023-2025">
        <div className="overflow-x-auto rounded-2xl border border-zinc-200/60">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-200 bg-zinc-50/50">
            ...
            <tbody className="divide-y divide-zinc-100">
              {historicalVoting.map((row) => (
                <tr key={row.year}>
                  <td className="px-4 py-2.5 font-semibold text-zinc-900 font-mono">
                    {row.year}
                  </td>
                  <td className="px-4 py-2.5">
                    <span className="flex items-center gap-1.5">
                      <span className={cn("h-2 w-2 rounded-full", row.boardElection >= 90 ? "bg-emerald-500" : row.boardElection >= 75 ? "bg-zinc-900" : "bg-amber-400")} />
                      <span className="font-mono font-medium text-zinc-700">{row.boardElection}%</span>
                    </span>
                  </td>
                  <td className="px-4 py-2.5">
                    <span className="flex items-center gap-1.5">
                      <span className={cn("h-2 w-2 rounded-full", row.compensation >= 90 ? "bg-emerald-500" : row.compensation >= 75 ? "bg-zinc-900" : row.compensation >= 60 ? "bg-amber-400" : "bg-red-400")} />
                      <span className="font-mono font-medium text-zinc-700">{row.compensation}%</span>
                    </span>
                  </td>
                  <td className="px-4 py-2.5">
                    <span className="flex items-center gap-1.5">
                      <span className="h-2 w-2 rounded-full bg-emerald-500" />
                      <span className="font-mono font-medium text-zinc-700">{row.auditorRatification}%</span>
                    </span>
                  </td>
                  <td className="px-4 py-2.5">
                    <span className="flex items-center gap-1.5">
                      <span className={cn("h-2 w-2 rounded-full", row.shareholderProposal >= 90 ? "bg-emerald-500" : row.shareholderProposal >= 75 ? "bg-zinc-900" : row.shareholderProposal >= 60 ? "bg-amber-400" : "bg-red-400")} />
                      <span className="font-mono font-medium text-zinc-700">{row.shareholderProposal}%</span>
                    </span>
                  </td>
                </tr>
              ))}
        ...
        {/* Trend alert */}
        <div className="mt-4 rounded-2xl bg-zinc-900 text-white p-5 shadow-sm">
          ...
          <p className="text-sm leading-relaxed">
            ...
            <span className="font-mono font-semibold text-zinc-400">78.5%</span> to{" "}
