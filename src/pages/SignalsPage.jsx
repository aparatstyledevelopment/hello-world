...
function FilterPill({ label, active, onClick }) {
  return (
    <button
      onClick={onClick}
      className={cn(
        "rounded-xl border px-3 py-1 text-[11px] font-medium tracking-wide transition-colors",
        active
...
function SignalPressureSummary({ filteredSignals }) {
  ...
  return (
    <div className="rounded-2xl border border-zinc-200/60 bg-white shadow-sm px-6 py-4 mb-6">
      <div className="flex items-center gap-4 mb-3">
        ...
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-1.5 text-xs">
            ...
            <span className={cn("font-mono font-bold", highCount > 0 ? "text-red-600" : "text-zinc-400")}>
              {highCount}
            </span>
          ...
          <span className="flex items-center gap-1.5 text-xs">
            ...
            <span className="font-mono font-bold text-zinc-600">{mediumCount}</span>
            <span className="text-zinc-400">Medium</span>
          </span>
          <span className="flex items-center gap-1.5 text-xs">
            <span className="h-2 w-2 rounded-full bg-zinc-400" />
            <span className="font-mono font-bold text-zinc-600">{lowCount}</span>
            <span className="text-zinc-400">Low</span>
          </span>
        </div>
        <span className="text-[11px] font-mono font-semibold text-zinc-400 ml-auto">
...
function SignalCard({ signal, navigate }) {
  ...
  return (
    <div
      className="rounded-2xl border border-zinc-200/60 transition-all overflow-hidden bg-white shadow-sm hover:border-zinc-300 hover:shadow-md"
    >
      {/* Card header */}
      <button
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            ...
            <span className="text-[11px] text-zinc-400">{relativeAge(signal.detectedAt)}</span>
          ...
          {/* Headline */}
          <h3 className={cn(
            "text-zinc-900 leading-snug",
            isHigh ? "text-base font-bold tracking-tight" : "text-sm font-semibold"
          )}>
            {signal.headline}
          </h3>
          ...
          <p className={cn(
            ...
            isHigh ? "text-sm text-zinc-600" : "text-xs text-zinc-500"
          )}>
          ...
          <div className="flex items-center gap-3 mt-2">
            <div className="flex items-center gap-1.5">
              <div className="flex items-center justify-center w-5 h-5 rounded-full bg-zinc-200 text-[8px] font-bold text-zinc-600">
                {inv?.name?.charAt(0) ?? "?"}
              </div>
              <span className="text-xs font-medium text-zinc-700">{inv?.name ?? "Unknown"}</span>
            </div>
          </div>
        </div>
      </button>
      ...
      {expanded && (
        <div className={cn(
          ...
          isHigh ? "border-red-100 bg-white" : "border-zinc-100 bg-white"
        )}>
          {/* Why this matters */}
          <div className="rounded-2xl bg-zinc-50 border border-zinc-100 p-4">
            <p className="text-sm font-semibold italic text-zinc-700 mb-2">Why this matters</p>
            <p className="text-sm text-zinc-600 leading-relaxed">{whyItMatters}</p>
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-zinc-700 leading-relaxed">{recommendedAction}</p>
          </div>
          ...
          {signal.parameters && signal.parameters.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {signal.parameters.map((p, i) => (
                <span
                  key={i}
                  className="inline-flex items-center gap-2 bg-zinc-50 border border-zinc-200 rounded-xl px-3 py-2 text-xs"
                >
                  <span className="text-zinc-400">{p.label}</span>
                  <span className="font-mono font-medium text-zinc-700">{p.value}</span>
                </span>
              ))}
            </div>
          )}
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-xl bg-zinc-900 px-4 py-2.5 text-xs font-bold text-white shadow-sm transition-colors hover:bg-zinc-800"
          >
  ...
  return (
    <div className="p-4 md:p-6 ">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-zinc-900 tracking-tight">Signals</h1>
