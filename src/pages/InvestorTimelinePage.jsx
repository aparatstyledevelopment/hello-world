...
export function InvestorTimelinePage() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      ...
      <Link
        to={`/investors/${id}`}
        className="inline-flex items-center gap-1.5 text-sm font-medium text-gray-400 transition-colors hover:text-black"
      >
        <ArrowLeft size={16} />
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-4 text-xs text-gray-500">
          <div>
            <span className="font-mono font-bold text-black text-lg">{events.length}</span>
            <span className="ml-1">events</span>
          </div>
          <div className="h-6 w-px bg-gray-200" />
          <div>
            <span className="font-mono font-bold text-gray-600 text-lg">{openSignals.length}</span>
            <span className="ml-1">signals</span>
          </div>
          <div className="h-6 w-px bg-gray-200" />
          <div>
            <span className="font-mono font-bold text-gray-600 text-lg">{openActions.length}</span>
        ...
        {filterOptions.map((opt) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn(
                "font-mono text-[10px]",
                filter === opt ? "text-gray-300" : "text-gray-400"
              )}>
                {count}
              </span>
            </button>
          ...
          {grouped.map((group) => (
            <div key={group.label}>
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-[11px] text-gray-400 font-mono">{group.events.length} events</span>
              </div>
                      ...
                      <div key={evt.id} className="relative">
                        ...
                        <div className="rounded-lg border border-gray-200 bg-white p-4 hover:border-gray-300 transition-colors">
                          <div className="flex items-center justify-between mb-1.5">
                            ...
                            <span className="text-xs text-gray-400">{formatDate(evt.date)}</span>
                          </div>
                          <p className="text-sm text-gray-700 leading-relaxed">{evt.description}</p>
                        </div>
                      </div>
                    );
                  })}
