    ...
    <div className="h-1.5 w-full rounded-full bg-zinc-100">
      <div
        className={cn(
          ...
          pct >= 80 ? "bg-red-400" : pct >= 50 ? "bg-amber-400" : "bg-emerald-500"
        )}
        style={{ width: `${pct}%` }}
      />
    </div>
    ...
    <div className="p-4 md:p-6 space-y-8">
      ...
      <div>
        <div className="flex items-center gap-3 mb-4">
          <h2 className="text-base font-bold text-zinc-900">Team Workload Overview</h2>
          ...
          {teamData.map((member) => (
            ...
            >
              <div className="flex items-start gap-4">
                {/* Avatar */}
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-zinc-100 text-sm font-bold text-zinc-700 flex-shrink-0">
                  {member.initials}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="inline-flex items-center rounded-full bg-zinc-100 text-zinc-700 border border-zinc-200 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.05em]">
                      {member.role}
                    </span>
                  </div>

                  <p className="text-xs text-zinc-500 mb-3">{member.focus}</p>
            ...
            <tbody className="divide-y divide-zinc-100">
              {coverageMap.map((row) => (
                <tr key={row.id} className={cn(
                  ...
                  <td className="px-4 py-2.5">
                    <span className="inline-flex items-center rounded-full bg-zinc-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.05em] text-zinc-600">
                      {row.type}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-2.5">
                    {row.openItems > 0 ? (
                      <span className="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-zinc-100 px-1.5 font-mono text-xs font-semibold text-zinc-700">
                        {row.openItems}
                      </span>
                    ) : (
                      <span className="font-mono text-xs text-zinc-300">0</span>
                    )}
                  </td>
                  <td className="px-4 py-2.5">
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-base font-bold text-zinc-900">Log & Actions</h2>
          <span className="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-zinc-900 px-1.5 font-mono text-[10px] font-bold text-white">
            {handoffNotes.length}
          </span>
        </div>
          ...
          {handoffNotes.map((note) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-3">
                <div className="flex h-7 w-7 items-center justify-center rounded-xl bg-zinc-100 text-[10px] font-bold text-zinc-700">
                  {note.fromInitials}
                </div>
                <span className="text-xs font-bold text-zinc-700">
                  {note.from}
                </span>
                <ArrowRightLeft size={12} className="text-zinc-300" />
                <div className="flex h-7 w-7 items-center justify-center rounded-xl bg-zinc-100 text-[10px] font-bold text-zinc-700">
                  {note.toInitials}
                </div>
                <span className="text-xs font-bold text-zinc-700">
              ...
              {member.signalsList.length > 0 ? (
                <div className="space-y-2">
                  {member.signalsList.map((sig) => (
                    <div
                      key={sig.id}
                      className="flex items-start gap-3 rounded-2xl bg-zinc-50 px-3 py-2.5"
                    >
  ...
  return (
    <div className="rounded-2xl border border-zinc-200/60 bg-white overflow-hidden shadow-sm">
      ...
      >
        ...
        <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-zinc-100 px-1.5 text-xs font-semibold text-zinc-600">
          {count}
        </span>
      </button>
